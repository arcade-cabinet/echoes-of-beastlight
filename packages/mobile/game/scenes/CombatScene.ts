...
export class CombatScene {
	...
	private arenaMesh: unknown | null = null;
	...
	clearCombat(): void {
		for (const [_, mesh] of this.playerMeshes) {
			(mesh as { dispose: (doNotRecurse?: boolean, disposeMaterialAndTextures?: boolean) => void }).dispose(false, true);
		}
		for (const [_, mesh] of this.enemyMeshes) {
			(mesh as { dispose: (doNotRecurse?: boolean, disposeMaterialAndTextures?: boolean) => void }).dispose(false, true);
		}
		if (this.arenaMesh) {
			(this.arenaMesh as { dispose: (doNotRecurse?: boolean, disposeMaterialAndTextures?: boolean) => void }).dispose(false, true);
			this.arenaMesh = null;
		}
